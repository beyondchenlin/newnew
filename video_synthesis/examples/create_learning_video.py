...
def create_top_title(text: str = "看视频学英语：") -> Tuple[str, str]:
    """
    创建顶部标题的所有相关设置
    ...
    Returns:
        Tuple[str, str]: (生成的图片路径, 对应的 filter_complex 命令)
    """
    # 文字配置
    config = {
        "font_size": 50,
        "color": "#00FF00",
        "y_offset": 0.04,
        "video_x": "W-w+10",  # 距离右边200像素
        "video_y": 130          # 距离顶部50像素
    }
    ...
    # 生成叠加命令
    overlay_command = f"[bg][1:v]overlay=x={config['video_x']}:y={config['video_y']}[with_top]"
    
    return text_img, overlay_command
...
def process_clip(input_video: str, output_video: str, item: Dict) -> bool:
    ...
    try:
        ...
        # 生成顶部文字图片和叠加命令
        logger.info("开始生成顶部文字...")
        top_text_img, top_overlay_command = create_top_title()
